...
private[hive] class SparkExecuteStatementOperation(
  ...
  private def execute(): Unit = withSchedulerPool {
    try {
      ...
      HiveThriftServer2.listener.postLiveListenerBus(SparkListenerStatementParsed(statementId, result.queryExecution.toString()))
      iter = {
  ...
  override def cancel(): Unit = {
    synchronized {
      if (!getStatus.getState.isTerminal) {
        ...
        HiveThriftServer2.listener.postLiveListenerBus(SparkListenerStatementCanceled(statementId))
      }
    }
  }
